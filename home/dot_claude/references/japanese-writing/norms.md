...
- **Mannered prose（技巧の誇示）**: a metaphor, personification, or flourish standing in for a direct statement（❌「ログが原因を語ってくれる」「本書は Rust の世界への招待状だ」 ✅「ログを見れば原因がわかる」「本書は Rust の入門書だ」）→ say what you mean; when a literal phrase exists, use it. The reader pays an extra step so the writer can perform
