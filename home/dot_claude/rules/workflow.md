...
- Spawning costs a fixed ~10K tokens before any work happens: the subagent re-reads the rules, CLAUDE.md, git status, and the target files the caller already has. Measured 2026-07-25: three agents that accomplished nothing burned 114K tokens. Delegate when the work dwarfs that cost, needs isolation from the caller's context, or when the generator must not be the evaluator. Stay inline for small, high-frequency tasks; on a 1M-context session, context hygiene alone does not justify a spawn
...
- After a worktree agent finishes, `git worktree list` and `git branch -v` must show the main tree untouched and the feature branch at the agent's commits; repoint with `git branch -f` if not
...
- Never claim completion without proof: the verification command runs **in the same message** as the claim, with full output and exit code. Diff behavior against main when relevant
